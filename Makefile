...
init-noninteractive: init-volumes
	@node cli/init --PRIVATE_NETWORK=$(PRIVATE_NETWORK) --NETWORK=$(NETWORK) --PEERS=$(PEERS) --PRIVATE_NODE_IPS=$(PRIVATE_NODE_IPS) --CORE_PUBLIC_IP_ADDRESS=$(CORE_PUBLIC_IP_ADDRESS) --BITCOIN_WIF=$(BITCOIN_WIF) --BTC_RPC_URI_LIST=$(BTC_RPC_URI_LIST) --INFURA_API_KEY=$(INFURA_API_KEY) --ETHERSCAN_API_KEY=$(ETHERSCAN_API_KEY)
	@rsync .env ${CORE_DATADIR}/.env
	@cp -rf config/traefik.toml ${CORE_DATADIR}/data/traefik/traefik.toml
