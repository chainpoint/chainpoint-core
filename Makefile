...
init-noninteractive: init-volumes
	@node cli/init --PRIVATE_NETWORK=$(PRIVATE_NETWORK) --CORE_PUBLIC_IP_ADDRESS=$(CORE_PUBLIC_IP_ADDRESS) --BITCOIN_WIF=$(BITCOIN_WIF) --INSIGHT_API_URI=$(INSIGHT_API_URI) --ETH_INFURA_API_KEY=$(ETH_INFURA_API_KEY) --ETH_ETHERSCAN_API_KEY=$(ETH_ETHERSCAN_API_KEY)
	@rsync .env ${CORE_DATADIR}/.env
	@cp -rf config/traefik.toml ${CORE_DATADIR}/data/traefik/traefik.toml
