...
async function getCalStateObjectsByAggIdsAsync(aggIds) {
  let aggIdData = aggIds.map(aggId => {
    return { aggId: aggId, data: null }
  })
  ...
  if (redis) {
    ...
    try {
      ...
      logger.warn(`Redis read error : getCalStateObjectsByAggIdsAsync : ${error.message}`)
    }

    // assign the redis results to the corresponding item in aggIdData
    aggIdData = aggIdData.map((item, index) => {
      item.data = redisResults[index]
      return item
    })
  ...
  if (redis) {
    ...
    dbResult.forEach(dbRow => {
      multi.set(
        `${CAL_STATE_KEY_PREFIX}:${dbRow.agg_id}`,
        JSON.stringify(dbRow),
        'EX',
        PROOF_STATE_CACHE_EXPIRE_MINUTES * 60
      )
    })
    ...
    try {
      ...
      logger.warn(`Redis write error : getCalStateObjectsByAggIdsAsync : ${error.message}`)
    }
  }
  return finalResult
}
...
async function getAnchorBTCAggStateObjectsByCalIdsAsync(calIds) {
  let calIdData = calIds.map(calId => {
    return { calId: calId, data: null }
  })
  ...
  if (redis) {
    ...
    try {
      ...
      logger.warn(`Redis read error : getAnchorBTCAggStateObjectsByCalIdsAsync : ${error.message}`)
    }

    // assign the redis results to the corresponding item in calIdData
    calIdData = calIdData.map((item, index) => {
      item.data = redisResults[index]
      return item
    })
  ...
  if (redis) {
    ...
    dbResult.forEach(dbRow => {
      multi.set(
        `${ANCHOR_BTC_AGG_STATE_KEY_PREFIX}:${dbRow.cal_id}`,
        JSON.stringify(dbRow),
        'EX',
        PROOF_STATE_CACHE_EXPIRE_MINUTES * 60
      )
    ...
    try {
      ...
      logger.warn(`Redis write error : getAnchorBTCAggStateObjectsByCalIdsAsync : ${error.message}`)
    }
  }
  return finalResult
}
...
async function getBTCTxStateObjectByAnchorBTCAggIdAsync(anchorBTCAggId) {
  ...
  if (redis) {
    try {
      ...
      logger.warn(`Redis read error : getBTCTxStateObjectByAnchorBTCAggIdAsync : ${error.message}`)
    }
  }
  ...
  if (redis) {
    try {
      ...
      logger.warn(`Redis write error : getBTCTxStateObjectByAnchorBTCAggIdAsync : ${error.message}`)
    }
  }
  return result
}
...
async function getBTCHeadStateObjectByBTCTxIdAsync(btcTxId) {
  ...
  if (redis) {
    try {
      ...
      logger.warn(`Redis read error : getBTCHeadStateObjectByBTCTxIdAsync : ${error.message}`)
    }
  }
  ...
  if (redis) {
    try {
      ...
      logger.warn(`Redis write error : getBTCHeadStateObjectByBTCTxIdAsync : ${error.message}`)
    }
  }
  return result
}
...
async function writeCalStateObjectsBulkAsync(stateObjects) {
  ...
  if (redis) {
    ...
    stateObjects.forEach(stateObj => {
      multi.set(
        `${CAL_STATE_KEY_PREFIX}:${stateObj.agg_id}`,
    ...
    try {
      ...
      logger.warn(`Redis write error : writeCalStateObjectsBulkAsync : ${error.message}`)
    }
  }
  return true
}
...
async function writeAnchorBTCAggStateObjectsAsync(stateObjects) {
  ...
  if (redis) {
    ...
    stateObjects.forEach(stateObj => {
      multi.set(
        `${ANCHOR_BTC_AGG_STATE_KEY_PREFIX}:${stateObj.cal_id}`,
        JSON.stringify(stateObj),
        'EX',
        PROOF_STATE_CACHE_EXPIRE_MINUTES * 60,
        'NX'
    ...
    try {
      ...
      logger.warn(`Redis write error : writeAnchorBTCAggStateObjectsAsync : ${error.message}`)
    }
  }
  return true
}
...
async function writeBTCTxStateObjectAsync(stateObject) {
  ...
  if (redis) {
    try {
      ...
      logger.warn(`Redis write error : writeBTCTxStateObjectAsync : ${error.message}`)
    }
  }
  return true
}
...
async function writeBTCHeadStateObjectAsync(stateObject) {
  ...
  if (redis) {
    try {
      ...
      logger.warn(`Redis write error : writeBTCHeadStateObjectAsync : ${error.message}`)
    }
  }
  return true
}
